...
import { logActions, organizationActions } from "./../../../actions";
...
class OrderLogs extends Component {
  ...
  getOrders = () => {
    ...
    dispatch(logActions.fetchOrders(accessToken, customerId));
    return true;
  };
  ...
  sendExportData = (allData) => {
    ...
    let formattedOrders = orders.map((order) => {
      ...
      this.state.exportHeaders.forEach((header) => {
        order[header.key] = _.isObject(order[header.key])
          ? order[header.key].props.orgcontent
          : order[header.key];
      });
  ...
  render() {
    ...
    return (
      <div id="outbound-order-logs-page">
        ...
        <div style={{ display: this.state.showFilter ? "none" : "block" }}>
          <div className="header-info">
            <div className="">
              <h3 id="order-logs-header">Order Logs</h3>
              <p id="order-logs-description">
                This list shows only orders placed in the Hub.
              </p>
            </div>
          </div>
